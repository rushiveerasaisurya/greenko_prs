...
export default function ClusterManagement() {
  ...
  const openEdit = (c) => { setEditingId(c.id); setForm({ name: c.name, region: c.region || '', clusterHead: c.clusterHead || '', description: '' }); setDrawerOpen(true); };

  const handleSave = () => {
    if (!form.name) { toast({ title: 'Cluster name is required', variant: 'destructive' }); return; }
    if (editingId) {
      updateCluster(editingId, { name: form.name, region: form.region, clusterHead: form.clusterHead });
      toast({ title: 'Cluster updated ✓' });
    } else {
      addCluster({ name: form.name, region: form.region, clusterHead: form.clusterHead });
      ...
      {drawerOpen && (<>
        ...
        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} className="fixed right-0 top-0 h-full w-full max-w-md bg-card z-50 shadow-xl border-l border-border overflow-y-auto">
          ...
          <div className="p-5 space-y-4">
            ...
            <div><label className="text-xs font-semibold text-muted-foreground">Cluster Head</label><select value={form.clusterHead} onChange={e => setForm({ ...form, clusterHead: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-lg border border-input bg-background text-sm"><option value="">Select</option>{clusterHeads.map(u => <option key={u.id} value={u.name}>{u.name}</option>)}</select></div>
