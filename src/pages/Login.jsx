...
export default function Login() {
  ...
  const handleInitialLogin = async (e) => {
    ...
    if (roles.length > 1) {
      ...
      if (singleRole === 'SITE_HEAD') {
        const sites = fetchedUser.sites || (fetchedUser.site ? [fetchedUser.site] : []);
        if (sites.length > 1) {
          setStep(4);
          return;
        }
      ...
      if (singleRole === 'CLUSTER_SAFETY_OFFICER') {
        setStep(3);
      } else {
        finalizeLogin(singleRole);
      }
    }
  };
  ...
  const handleRoleSelection = (e) => {
    e.preventDefault();
    // Don't switch yet, just transition
    
    // Check if site selection is needed
    if (selectedRole === 'SITE_HEAD') {
      const sites = tempUser?.sites || (tempUser?.site ? [tempUser?.site] : []);
      if (sites.length > 1) {
        setStep(4);
        return;
      } else {
        // Just store for finalizeLogin
  ...
  const finalizeLogin = (roleToUse) => {
    const updates = { activeRole: roleToUse };
    if (roleToUse === 'SITE_HEAD' && selectedSite) updates.site = selectedSite;
    if ((roleToUse === 'CLUSTER_HEAD' || roleToUse === 'CLUSTER_SAFETY_OFFICER') && selectedCluster) updates.cluster = selectedCluster;
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }}>
              <motion.h2 variants={fadeUp} className="text-3xl font-extrabold font-display text-foreground mb-2">
                {step === 1 ? 'Welcome back' : step === 2 ? 'Choose Your Role' : step === 3 ? 'Select Cluster' : 'Select Site'}
              </motion.h2>
              <motion.p variants={fadeUp} className="text-muted-foreground text-sm mb-8">
                {step === 1 ? 'Sign in to access your safety dashboard' : step === 2 ? `Hello, ${tempUser?.name}! Pick a role to continue.` : step === 3 ? 'Which cluster would you like to explore?' : 'Which site are you monitoring today?'}
              </motion.p>
              ...
              {step === 1 ? (
                ...
                <form onSubmit={handleRoleSelection} className="space-y-4">
                  ...
                  <div className="text-center mt-2">
                    <button type="button" onClick={() => setStep(1)} className="text-xs text-muted-foreground hover:text-foreground hover:underline transition-colors">← Back</button>
                  </div>
                </form>
              ) : step === 3 ? (
                ...
                <form onSubmit={handleSiteSelection} className="space-y-4">
                  <div className="grid grid-cols-1 gap-3 max-h-[40vh] overflow-y-auto pr-1 pb-1">
                    {(tempUser?.sites || (tempUser?.site ? [tempUser?.site] : [])).map((s, idx) => (
                      <motion.button
                        key={s} type="button"
                        initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08, duration: 0.3 }}
                        onClick={() => setSelectedSite(s)}
                        className={`p-4 rounded-xl border-2 text-left transition-all duration-200 flex items-center justify-between hover-lift
                          ${selectedSite === s ? 'border-primary bg-primary/8 shadow-md shadow-primary/10' : 'border-border hover:border-muted-foreground bg-card'}`}
                      >
                        <span className="block font-bold text-sm text-foreground">{s}</span>
                        {selectedSite === s && (
                          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-5 h-5 rounded-full bg-primary flex items-center justify-center">
                            <div className="w-2 h-2 bg-white rounded-full" />
                          </motion.div>
                  ...
                  <div className="pt-2">
                    <Button type="submit" disabled={!selectedSite} className="w-full h-12 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-display font-bold text-sm shadow-lg shadow-primary/20 btn-press">
                      Select Site →
                    </Button>
                  </div>
                  <div className="text-center mt-2">
